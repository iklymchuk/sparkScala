...
/**
  * Created by iklymchuk on 1/19/17.
  */
object SocialGraphs extends Serializable{
  ...
  def main(args: Array[String]) {
    ...
    val sc = new SparkContext("local[*]", "SocialGraphs")

    // Build up a hero ID -> name RDD
    val names = sc.textFile("inputData/Marvel-names.txt")
    val namesRdd = names.flatMap(parseNames)

    // Load up the superhero co-apperarance data
    val lines = sc.textFile("inputData/Marvel-graph.txt")
